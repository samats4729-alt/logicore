...
describe('Знание ИИ-гида о платформе', () => {
    ...
    describe('границы того, что гид говорит', () => {
        it('цифры разрешены только из блока данных, а не по памяти', () => {
            // Раньше гид не видел данных вовсе, и правило было простое:
            // цифр не называть. Теперь он видит сводку по компании, и
            // запрет сменился на границу — говорить только то, что в
            // сводке есть. Выдуманная сумма выглядит как настоящая, и
            // человек примет по ней решение.
            expect(PLATFORM_KNOWLEDGE).toMatch(/только из блока «Данные компании собеседника»/);
            expect(PLATFORM_KNOWLEDGE).toMatch(/По памяти не называй ничего/);
        });

        it('на вопрос о долге конкретного контрагента гид ведёт во «Взаиморасчёты»', () => {
            // Сумм по контрагентам в сводке нет — только счётчики. Без этого
            // правила гид назовёт общее число неоплаченных счетов так, будто
            // это и есть долг названной компании.
            expect(PLATFORM_KNOWLEDGE).toMatch(/сколько мне должен такой-то/);
        });
