...
- /company/billing — «Подписка»: до какого числа оплачен кабинет, сколько стоит месяц и как продлить. Открывается из меню под именем человека, доступна руководителю и экспедитору
...
Мастер создания заявки (/company/orders/create) — три шага, и на каждом свои
поля. Шаг открывается только после того, как предыдущий заполнен:
- пилюли шагов: [data-guide='wizard-step-0'], [data-guide='wizard-step-1'], [data-guide='wizard-step-2']
- поле «Заказчик» (шаг 1): [data-guide='wizard-customer']
- поле «Перевозчик» (шаг 1): [data-guide='wizard-carrier']
- «Дата и время погрузки» (шаг 2, без неё дальше не пускает): [data-guide='wizard-pickup-date']
- «Описание груза» (шаг 3): [data-guide='wizard-cargo']
- «Далее»: [data-guide='wizard-next']
- «Назад»: [data-guide='wizard-back']
- «Создать заявку» (только на последнем шаге): [data-guide='wizard-submit']

Карточка счёта или акта (открывается из журнала кликом по номеру):
- «Провести»: [data-guide='doc-post']
- «Провести и закрыть»: [data-guide='doc-post-close']
- меню остальных действий — отправить контрагенту, создать акт на основании,
  скопировать ссылку, отменить: [data-guide='doc-more']

Про пункты выпадающих меню. У «Отправить», «Создать акт на основании» и
прочего внутри [data-guide='doc-more'] своих селекторов нет. Доведи до самой
кнопки меню, а нужный пункт назови словами в say.
`;
...
\`\`\`
Правила для steps:
- Каждый шаг = одно действие пользователя. say — короткая команда (что нажать или что заполнить).
- Используй ТОЛЬКО селекторы из списка выше.
- Веди до конца задачи, а не до страницы. «Создать заявку» — это не «откройте Заявки», а весь путь: журнал, кнопка создания, стороны сделки, «Далее», маршрут и дата, «Далее», груз, «Создать заявку». Длинный маршрут — нормально, десяток шагов не предел.
- Если на шаге надо не нажать, а заполнить поле или выбрать значение, добавь "fill": true. Такой шаг не перескакивает от касания — человек заполняет и сам жмёт «Дальше». Без этого маршрут по форме убегает вперёд, пока поля пустые.
- Шаг, который появляется только после предыдущего действия (следующий шаг мастера, кнопка в открывшейся карточке), всё равно ставь: подсветка ждёт появления элемента.
        ...
        try {
            const res = await fetch('https://api.deepseek.com/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    temperature: 0.3,
                    // Длинный маршрут — десяток шагов — упирался в этот предел
                    // и обрывался на середине JSON. Разобрать такой блок
                    // нельзя, и подсветка не запускалась вовсе: на большую
                    // задачу гид отвечал текстом и молча терял шаги.
                    max_tokens: 1600,
                    stream: false,
                }),
            });
