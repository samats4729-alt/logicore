...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f5f5f5;">
    ...
    async sendUpdatesPendingApprovalEmail(to: string, count: number): Promise<void> {
        const frontendUrl = (this.configService.get<string>('FRONTEND_URL') || 'http://localhost:3000').replace(/\/$/, '');
        const link = `${frontendUrl}/admin/updates`;

        const subject = 'Новые нововведения на платформе ожидают подтверждения';
        const html = `
<!DOCTYPE html>
<html>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f5f5f5;">
    <div style="max-width: 480px; margin: 40px auto; background: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">
        <div style="background: linear-gradient(135deg, #1677ff 0%, #4096ff 100%); padding: 32px 24px; text-align: center;">
            <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 700; letter-spacing: -0.5px;">LogiCore</h1>
            <p style="color: rgba(255,255,255,0.85); margin: 8px 0 0; font-size: 14px;">Панель администратора</p>
        </div>
        <div style="padding: 32px 24px;">
            <h2 style="margin: 0 0 16px; font-size: 20px; color: #1a1a1a;">Новые нововведения</h2>
            <p style="color: #595959; font-size: 15px; line-height: 1.6; margin: 0 0 24px;">
                ИИ проанализировал новые коммиты и подготовил черновики анонсов нововведений (количество: <strong>${count}</strong>).
                Пожалуйста, проверьте и утвердите их в панели управления:
            </p>
            <div style="text-align: center; margin: 32px 0;">
                <a href="${link}" 
                   style="display: inline-block; background: #1677ff; color: #ffffff; text-decoration: none; padding: 14px 40px; border-radius: 8px; font-size: 16px; font-weight: 600; letter-spacing: 0.3px;">
                    Утвердить нововведения
                </a>
            </div>
            <hr style="border: none; border-top: 1px solid #f0f0f0; margin: 24px 0;">
            <p style="color: #bfbfbf; font-size: 12px; line-height: 1.5; margin: 0;">
                Это автоматическое письмо для администраторов платформы LogiCore.
            </p>
        </div>
    </div>
</body>
        ...
        if (!this.resend) {
            this.logger.log(`📧 [DEV] Updates pending approval email for ${to}. Count: ${count}`);
        ...
        try {
            ...
            this.logger.log(`📧 Updates pending approval email sent to ${to}`);
        } catch (error: any) {
            this.logger.error(`Failed to send updates pending approval email to ${to}: ${error.message}`);
        }
    }
}
