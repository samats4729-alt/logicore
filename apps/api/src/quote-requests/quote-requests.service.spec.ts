...
describe('Запросы на расчёт: сервис', () => {
    ...
    /**
     * Главная жалоба клиента платформы. У «Шымкент пиво» по маршруту запрос
     * был, менеджер завёл новый и выбрал «Шымкентский пивзавод» — панель
     * написала «по этому направлению данных нет». Направление-то одно.
     *
     * Поэтому список один на направление, по всем клиентам, а чей был
     * запрос — написано в строке.
     */
    describe('память по направлению', () => {
        ...
        it('отбор идёт по направлению, а не по карточке клиента', async () => {
            const { service, prisma } = создать();
            ...
            // В отборе клиента нет: иначе запрос соседнего клиента по тому
            // же маршруту не нашёлся бы никогда.
            const where = prisma.quoteRequest.findMany.mock.calls[0][0].where;
            expect(JSON.stringify(where)).not.toContain('customerCompanyId');
        });

        it('запрос другого клиента по этому направлению виден', async () => {
            ...
            // По направлению история есть, и её видно — вместе с тем, чей
            // это был запрос.
            expect(result.direction.count).toBe(1);
            expect(result.direction.items[0].customerName).toBe('Шымкентский пивзавод');
        });

        it('в списке видно запросы всех клиентов по этому направлению', async () => {
            // Ровно то, чего не хватало: «по данному направлению никаких
            // данных не было», хотя у соседнего клиента запрос был.
            const { service } = создать({
                prisma: {
                    quoteRequest: {
                        findMany: jest.fn().mockResolvedValue([
            ...
            expect(result.direction.count).toBe(2);
            expect(result.direction.range?.customerFrom).toBe(90_000);
            expect(result.direction.range?.customerTo).toBe(130_000);
            // Вывод об исходе — по-прежнему про выбранного клиента.
            expect(result.last?.id).toBe('свой');
        });
        ...
        it('без названного маршрута история не показывается вовсе', async () => {
            ...
            expect(result.direction.count).toBe(0);
        });
