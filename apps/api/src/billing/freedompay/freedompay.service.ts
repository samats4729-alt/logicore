...
export class FreedomPayService {
    ...
    private async post(url: string, params: Record<string, string>): Promise<Record<string, string>> {
        ...
        try {
            ...
            // Само по себе «fetch failed» не говорит ничего: под ним и
            // несуществующий адрес, и закрытый порт, и просроченный
            // сертификат. Настоящая причина лежит в `cause`, иногда на два
            // уровня вглубь, — и именно она отвечает, что чинить.
            throw new Error(`Не удалось связаться с ${url}: ${причинаСети(error)}`);
        } finally {
            clearTimeout(таймер);
        }
    }
...
/**
 * Развернуть сетевую ошибку до чего-то осмысленного.
 *
 * `fetch` в Node на любую сетевую беду отвечает одинаково — «fetch failed».
 * Под этими двумя словами прячется всё: несуществующее имя узла, закрытый
 * порт, отвергнутый сертификат, оборванное соединение. Настоящая причина
 * лежит в цепочке `cause`, и без неё разбирательство упирается в стену:
 * непонятно даже, чинить адрес, сеть или сертификат.
 *
 * Собираем всю цепочку, потому что глубина у неё разная: у отказа DNS
 * причина на первом уровне, у ошибки TLS бывает на втором.
 */
function причинаСети(error: any): string {
    const куски: string[] = [];
    let текущая: any = error;
    for (let глубина = 0; текущая && глубина < 4; глубина += 1) {
        const код = текущая.code || текущая.errno;
        const текст = текущая.message;
        // «fetch failed» само по себе ничего не добавляет — его пропускаем,
        // но только если под ним есть что-то содержательное.
        if (текст && !(текст === 'fetch failed' && текущая.cause)) {
            куски.push(код && !String(текст).includes(String(код)) ? `${текст} (${код})` : текст);
        } else if (код) {
            куски.push(String(код));
        }
        текущая = текущая.cause;
    }
    // Имя узла, если оно есть: именно оно отвечает на «а туда ли мы идём».
    const узел = error?.cause?.hostname || error?.hostname;
    if (узел && !куски.some((к) => к.includes(узел))) куски.push(`узел ${узел}`);

    return куски.length ? куски.join(' ← ') : String(error);
}
