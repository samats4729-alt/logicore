...
import {
    ...
    Modal, Form, Input, InputNumber, Select, DatePicker, message, Timeline, Space, Spin, Divider
} from 'antd';
import {
...
export default function OrderDetailPage() {
    ...
    const incomeColumns = [
        ...
        { title: 'Категория', dataIndex: 'category', key: 'cat', width: 140, render: (c: string) => incomeCategories.find(x => x.value === c)?.label || c },
        { title: 'Описание', dataIndex: 'description', key: 'desc', ellipsis: true },
        { title: 'Сумма ₸', dataIndex: 'amount', key: 'amount', width: 120, align: 'right' as const, render: (a: number) => <Text strong style={{ color: '#389e0d' }}>{fmt(a)}</Text> },
        { title: '', key: 'actions', width: 50, render: (_: any, r: any) => (
            <Button size="small" danger icon={<DeleteOutlined />} onClick={() => handleDeleteIncome(r.id)} />
        )},
    ];

    const expenseColumns = [
        { title: 'Дата', dataIndex: 'date', key: 'date', width: 100, render: (d: string) => dayjs(d).format('DD.MM.YY') },
        { title: 'Категория', dataIndex: 'category', key: 'cat', width: 140, render: (c: string) => expenseCategories.find(x => x.value === c)?.label || c },
        { title: 'Описание', dataIndex: 'description', key: 'desc', ellipsis: true },
        { title: 'Сумма ₸', dataIndex: 'amount', key: 'amount', width: 120, align: 'right' as const, render: (a: number) => <Text strong style={{ color: '#cf1322' }}>{fmt(a)}</Text> },
        { title: '', key: 'actions', width: 50, render: (_: any, r: any) => (
            <Button size="small" danger icon={<DeleteOutlined />} onClick={() => handleDeleteExpense(r.id)} />
        )},
    ];
