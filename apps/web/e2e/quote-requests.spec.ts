...
test.describe('Запросы на расчёт', () => {
    ...
    test('память по клиенту и маршруту появляется до того, как названа цена', async ({ page }) => {
        ...
        // Проверяется появление панели памяти, а не наличие истории. На стенде
        // с одним посторонним запросом история по ЭТОМУ клиенту и маршруту не
        // обязана существовать: тогда панель честно говорит, что запросов ещё
        // не было. Требовать «машину нашли за» безусловно — значит требовать
        // от стенда подходящих данных, и тест падал от того, что в базе просто
        // появилась чужая строка.
        await expect(
            form.getByText(/машину нашли за|запросов ещё не было/),
